	...
	return func(c *gin.Context) {
		defer func() {
			if err := recover(); err != nil {
				...
				ExtractLogger(c).WithField("stack", string(stack)).WithField("httprequest", string(httprequest)).Errorf("[Zeushttp Recovery] panic recovered:%s", err)
				e, ok := err.(error)
				if ok {
					ErrorResponse(c, e)
					return
				}
				ErrorResponse(c, zeuserrors.ECodeInternal.ParseErr(fmt.Sprint(err)))
			}
		}()
		c.Next()
	}
