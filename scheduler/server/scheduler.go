...
func (sch *JobScheduler) handleJobUpdateEvent(event *JobChangeEvent) {
	...
	if _, ok = sch.schedulePlans[jobConf.Id]; !ok {
		...
		log.Printf("the job conf:%#v change job create event", jobConf)
	...
	if jobConf.Status == JobStopStatus {
		...
		log.Printf("the job conf:%#v status is stop must delete from the schedule plan ", jobConf)
		delete(sch.schedulePlans, jobConf.Id)
		return
	}

	if schedule, err = cron.Parse(jobConf.Cron); err != nil {
		log.Printf("the job conf:%#v  parse the cron error:%#v", jobConf, err)
		return
	}
...
func (sch *JobScheduler) handleJobDeleteEvent(event *JobChangeEvent) {
	...
	if plan.Version > jobConf.Version && jobConf.Version != -1 {
		log.Printf("the job conf:%#v version:%d <  schedule plan:%#v,version:%d", jobConf, jobConf.Version, plan, plan.Version)
		return
	}
	log.Printf("the job conf:%#v delete a  schedule plan:%#v", jobConf, plan)
	delete(sch.schedulePlans, jobConf.Id)
...
func (sch *JobScheduler) createJobPlan(event *JobChangeEvent) {
	...
	if _, ok := sch.schedulePlans[jobConf.Id]; ok {
		log.Printf("the job conf:%#v exist", jobConf)
		return
	}

	if jobConf.Status == JobStopStatus {

		log.Printf("the job conf:%#v status is stop", jobConf)
		return
	}

	if schedule, err = cron.Parse(jobConf.Cron); err != nil {
		log.Printf("the job conf:%#v cron is error exp ", jobConf)
		return
	}
...
func (sch *JobScheduler) loopSchedule() {
	...
	for {
		...
		log.Printf("the durationTime :%d", durationTime)
...
func (sch *JobScheduler) trySchedule() time.Duration {
	...
	for _, plan := range sch.schedulePlans {
		...
		if scheduleTime.Before(now) && sch.node.state == NodeLeaderState {
			log.Printf("schedule execute the plan:%#v", plan)
...
func (sch *JobScheduler) trySync() {
	...
	if sch.syncStatus == true {
		log.Printf("the sync event is syncing ....")
		return
	}

	now := time.Now()
	log.Printf("start sync the  schedule plan ....")
	...
	for id, plan := range sch.schedulePlans {
		...
		if !sch.existPlan(id, jobConfs) {
			log.Printf("sync the schedule plan %v must delete", plan)
			delete(sch.schedulePlans, id)
		}
	}

	log.Printf("finish sync the  schedule plan use【%dms】....", time.Now().Sub(now)/time.Millisecond)
...
func (sch *JobScheduler) handleJobConfSync(conf *JobConf) {
	...
	if plan, exist = sch.schedulePlans[conf.Id]; !exist {
		...
		if conf.Status == JobRunningStatus {
			log.Printf("sync the schedule plan the job conf: %v must create", conf)
		...
		if plan.Version < conf.Version {
			log.Printf("sync the schedule plan %v must update", plan)
...
func (sch *JobScheduler) notify(state int) {
	log.Printf("found the job :{} state notify state:%d", sch.node, state)
	if state == NodeLeaderState {
		log.Printf("found the job :{} state notify state:%d,must sync the job schedule plan", sch.node, state)
		sch.trySync()
	}
}
