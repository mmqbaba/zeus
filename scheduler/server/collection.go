...
func (c *JobCollection) handleJobExecuteStatusCollectionEvent(event *KeyChangeEvent) {
	...
	case KeyCreateChangeEvent:
		...
		if err != nil {
			log.Printf("UParkJobExecuteSnapshot:%s fail,err:%#v ", event.Value, err)
	...
	for {
		...
		case <-timer.C:
			...
			if err != nil {
				...
				log.Printf("collection loop err:%v ", err)
				continue
			}
			...
			for pos := 0; pos < len(keys); pos++ {
				...
				if err != nil {
					log.Printf("UParkJobExecuteSnapshot:%s fail,err:%#v ", values[pos], err)
