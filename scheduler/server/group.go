...
RETRY:
	...
	for i := 0; i < len(keys); i++ {
		...
		if err != nil {
			log.Printf("upark the group conf error:%#v", err)
			continue
		}
...
func (mgr *JobGroupManager) addGroup(name, path string) {
	...
	log.Printf("add a new group:%s,for path:%s", name, path)
...
func (mgr *JobGroupManager) deleteGroup(path string) {
	...
	log.Printf("delete a  group:%s,for path:%s", group.name, path)
}
...
RETRY:
	...
	for i := 0; i < len(keys); i++ {
		...
		if value == "" {
			log.Printf("the client value is nil for path:%s", path)
			continue
		}
...
func (group *Group) addClient(name, path string) {
	...
	if _, ok := group.clients[path]; ok {
		log.Printf("name:%s,path:%s,the client exist", name, path)
		return
	}
...
func (group *Group) deleteClient(path string) {
	...
	if client, ok = group.clients[path]; !ok {
		log.Printf("path:%s,the client not  exist", path)
		return
	}
